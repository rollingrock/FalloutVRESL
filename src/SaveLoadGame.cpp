...
struct BGSSaveLoadGameCTORHook
{
	...
	{
		REL::Relocation<std::uintptr_t> target{ REL::Offset(0x0d252a0) };  // Skyrim was 0x5819f0
		pstl::write_thunk_call<BGSSaveLoadGameCTORHook>(target.address() + 0x14B);  // 0x144
		pstl::write_thunk_call<BGSSaveLoadGameCTORHook>(target.address() + 0x15F);  // 0x158
	}
